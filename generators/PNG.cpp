...
public:
	...
	CTYPE* generate(std::vector<std::string>&);
};
...
public:
	...
	uint_class(bool small, std::vector<uint> known_values = {}) : small(small), known_values(known_values) {}
...
public:
	...
	PNG_CHUNK* generate(std::vector<std::string>&, bool evil_type);
};

std::vector<char> ReadByte_values;
std::vector<double> ReadDouble_values;
std::vector<float> ReadFloat_values;
std::vector<hfloat> ReadHFloat_values;
std::vector<int> ReadInt_values;
std::vector<int64> ReadInt64_values;
std::vector<int64> ReadQuad_values;
std::vector<short> ReadShort_values;
std::vector<uchar> ReadUByte_values;
std::vector<uint> ReadUInt_values;
std::vector<uint64> ReadUInt64_values;
std::vector<uint64> ReadUQuad_values;
std::vector<ushort> ReadUShort_values;
...
public:
	...
	globals_class() :
		...
		physPixelPerUnitX(true),
		physPixelPerUnitY(true),
		phys(PNG_CHUNK_PHYS_phys_instances),
		bgColorPaletteIndex(true),
		bgGrayscalePixelValue(true),
		bgColorPixelRed(true),
		bgColorPixelGreen(true),
		bgColorPixelBlue(true),
		bkgd(PNG_CHUNK_BKGD_bkgd_instances),
		sbitRed(true),
		sbitGreen(true),
		sbitBlue(true),
		sbitGraySource(true),
		sbitGrayAlphaSource(true),
		sbitGrayAlphaSourceAlpha(true),
		sbitColorRed(true),
		sbitColorGreen(true),
		sbitColorBlue(true),
		sbitColorAlphaRed(true),
		sbitColorAlphaGreen(true),
		sbitColorAlphaBlue(true),
		sbitColorAlphaAlpha(true),
...
CTYPE* CTYPE::generate(std::vector<std::string>& chunk_types) {
	if (generated == 1) {
		CTYPE* new_instance = new CTYPE(instances);
		new_instance->generated = 2;
		return new_instance->generate(chunk_types);
	}
	if (!generated)
		generated = 1;

//	GENERATE_VAR(ctype, ::g->ctype.generate());
	ctype_exists = true;
	start_generation("cname");
	memcpy(cname_var, ::g->cname.generate(4, chunk_types).c_str(), 4);
	cname_exists = true;
	end_generation();
	return this;
}
...
std::string generate_data(uint32 width, uint32 height, PNG_COLOR_SPACE_TYPE color, ubyte bit_depth, PNG_INTERLACE_METHOD interlace) {
	...
	int bits_per_pixel = channels_per_pixel[color] * bit_depth;
	...
	if (!interlace) {
		for (unsigned y = 0; y < height; ++y) {
			...
			unsigned char filter_type = file_acc.rand_int(5, [&current_byte](unsigned char* file_buf) -> long long { return current_byte; } );
			data += filter_type;
	...
	if (!file_acc.generate) {
		...
		for (int l = 0; l < 10; ++l)
			if (compress_data(data, l) == compressed) {
				parse = [l](unsigned char* file_buf) -> long long { return l + 1; };
				break;
			}
		assert_cond(!!parse, "failed to find working compression level");
	}
	int level = file_acc.rand_int(11, parse) - 1;
...
PNG_CHUNK* PNG_CHUNK::generate(std::vector<std::string>& chunk_types, bool evil_type) {
	if (generated == 1) {
		PNG_CHUNK* new_instance = new PNG_CHUNK(instances);
		new_instance->generated = 2;
		return new_instance->generate(chunk_types, evil_type);
	}
	if (!generated)
		generated = 1;
	...
	pos_start = FTell();
	bool evil = file_acc.set_evil_bit(evil_type);
	GENERATE_VAR(type, ::g->type.generate(chunk_types));
	...
	int64 pos_end = FTell();
	unsigned correct_length = pos_end - pos_start - 4;
...
void generate_file() {
	...
	std::vector<std::string> chunk_types = { "IHDR" };
	GENERATE(chunk, ::g->chunk.generate(chunk_types, false));

	chunk_types = { "iCCP", "sRGB", "sBIT", "gAMA", "cHRM", "pHYs", "sPLT", "tIME", "zTXt", "tEXt", "iTXt", "eXIf", "oFFs", "pCAL", "sCAL", "acTL", "fcTL", "fdAT", "fRAc", "gIFg", "gIFt", "gIFx", "sTER" };
	while (file_acc.rand_int(4, [](unsigned char* file_buf) -> long long {
			return strncmp((char*)&file_buf[4], "PLTE", 4) == 0 ||
			       strncmp((char*)&file_buf[4], "IDAT", 4) == 0 ||
			       strncmp((char*)&file_buf[4], "tRNS", 4) == 0 ||
			       strncmp((char*)&file_buf[4], "hIST", 4) == 0 ||
			       strncmp((char*)&file_buf[4], "bKGD", 4) == 0 ? 3 : 0;
		} ) != 3) {
		GENERATE(chunk, ::g->chunk.generate(chunk_types, true));
	};

	chunk_types = { "PLTE" };
	switch (::g->chunk()[0]->ihdr().color_type()) {
	case TrueColor:
	case AlphaTrueColor:
		if (file_acc.rand_int(2, [](unsigned char* file_buf) -> long long {
				return strncmp((char*)&file_buf[4], "PLTE", 4) == 0 ? 1 : 0;
			} ))
			GENERATE(chunk, ::g->chunk.generate(chunk_types, false));
		break;
	case Indexed:
	...
	chunk_types = { "tRNS", "hIST", "bKGD", "pHYs", "sPLT", "tIME", "zTXt", "tEXt", "iTXt", "eXIf", "oFFs", "pCAL", "sCAL", "acTL", "fcTL", "fdAT", "fRAc", "gIFg", "gIFt", "gIFx", "sTER" };
	while (file_acc.rand_int(4, [](unsigned char* file_buf) -> long long {
			return strncmp((char*)&file_buf[4], "IDAT", 4) == 0 ? 3 : 0;
		} ) != 3) {
		GENERATE(chunk, ::g->chunk.generate(chunk_types, true));
	};
	...
	chunk_types = { "tIME", "zTXt", "tEXt", "iTXt", "eXIf", "oFFs", "pCAL", "sCAL", "acTL", "fcTL", "fdAT", "fRAc", "gIFg", "gIFt", "gIFx", "sTER" };
	while (file_acc.rand_int(4, [](unsigned char* file_buf) -> long long {
			return strncmp((char*)&file_buf[4], "IEND", 4) == 0 ? 3 : 0;
		} ) != 3) {
		GENERATE(chunk, ::g->chunk.generate(chunk_types, true));
	};
