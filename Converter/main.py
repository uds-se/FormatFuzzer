...
def run_tests_on_all(logger, file_path, file_names=None, filter_diffs=True):
    ...
    if file_names is not None:
        ...
        tb.run_multi_format_parse_test(file_names, tb.provide_wild_files(file_path, logger), filter_diffs, logger)
    else:
        files = listdir(file_path)
        log.info(f"formats to be run: {files}")
        tb.run_multi_format_parse_test(files, tb.provide_wild_files(file_path, logger), filter_diffs, logger)


def convert_all(logger, file_path, file_names=None, filter_diffs=True):
...
def main():
    ...
    args.run(logger, args.file_path, args.formats, args.filter_diffs)
