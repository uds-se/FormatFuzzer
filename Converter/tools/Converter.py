...
class enums(Converter):
    ...
    def gen_single_enum(self, key, enumerations, type="<ubyte>"):
        ...
        for k in keys[0:-1]:
            lines.append(
                "  " +
                (values[k] if "id" not in values[k] else values[k]["id"]) +
                " = " + str(hex(k)) + "," +
                ("" if "doc" not in values[k] else "     // " +
                 (values[k]["doc"]).strip().replace("\n", "\n     //")))
        lines.append(
            "  " + (values[keys[-1]] if "id" not in
                    values[keys[-1]] else values[keys[-1]]["id"]) + " = " +
            str(hex(keys[-1])) + "" +
            ("" if "doc" not in values[keys[-1]] else "      // " +
             (values[keys[-1]]["doc"]).strip().replace("\n", "\n     //")))
        lines.append("};")
        return lines
