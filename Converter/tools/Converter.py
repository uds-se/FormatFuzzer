...
class Converter(object):
    ...
    def register_instance(
        self, name, value, containing_type
    ):  # registers instance in "global" table during parsing time
        try:
            self.root.global_instance_table[str(containing_type)][str(
                name)] = {
                    "VALUE": value,
                    "IMPLEMENTED": False
                }
        except:
            self.root.global_instance_table[str(containing_type)] = {
                str(name): {
                    "VALUE": value,
                    "IMPLEMENTED": False
                }
    ...
    def register_type(
        ...
        if not param_needed:
            self.root.global_type_table[str(name)] = {
                "VALUE": value,
                "IMPLEMENTED": False,
                "PARAM": param_needed
            }
    ...
    def register_enum(
            self, name,
            value):  # registers enum in "global" table during parsing time
        self.root.global_enum_table[str(name)] = {
            "VALUE": value,
            "IMPLEMENTED": False
        }
    ...
    def lookup_instance(
        ...
        if check_if_implemented:
            ...
            getter = "VALUE"
        try:
            return self.root.global_instance_table[str(containing_type)][str(
                name)][getter]
        except:
    ...
    def parse_subtree(self, name=None):
        ...
        for this_level_key in self.this_level_keys:
            ...
            if local_key is not None:
                ...
                self.subtrees[local_key] = globals()[local_key](
                    self.input[this_level_key], self, self.root, name=name)
                self.subtrees[local_key].parse_subtree(name=name)

    def generate_code_toplevel(self):
        if "enums" in self.subtrees.keys():
            self.output_enums.extend(
                self.subtrees["enums"].generate_code(called_lowlevel=False))
        self.output_types.extend(
            self.subtrees["types"].generate_code(called_lowlevel=False))
        self.output_seqs.extend(
            self.subtrees["seq"].generate_code(called_lowlevel=False))
    ...
    def generate_code(self,
        ...
        if "instances" in self.this_level_keys:
            for x in self.input["instances"].keys():
                self.mark_as_implemented("instance",
                                         x,
                                         value=False,
        ...
        for this_level_key in self.this_level_keys:
            ...
            if local_key is not None:
                if True:  # SWITCH BETWEEN SUPPRESSION OF "UNIMPLEMENTED" ERRORS
                    try:
                        self.output.extend(
                            self.subtrees[local_key].generate_code(
                                size, called_lowlevel))
    ...
    def expr_resolve(self,
                     expr,
                     translate_condition_2_c=False,
        ...
        for to_be_rep in operator_replacement_dict.keys():
            expr = expr.replace(to_be_rep,
                                operator_replacement_dict[to_be_rep])
        # for element in re.split('(\W+)', expr):  # replacing f**kin 0xffff_ffff with 0xffffffff
        for element in re.split(
                '[^a-zA-Z0-9_.]+',
                expr):  # replacing f**kin 0xffff_ffff with 0xffffffff
            try:
                try:
    ...
    def resolve_datatype(self, kaitype, getsize=False, getendian=False):
        ...
        match = re.match(
            r'(?P<parsed_type>[a-zA-Z])(?P<parsed_size>[0-9])(?P<parsed_endian>[a-zA-Z]*)',
            kaitype)
        if match is None:
            return None
...
class enums(Converter):
    ...
    def parse_subtree(self, name=None):
        for this_level_key in self.this_level_keys:
            ...
            if local_key is not None:
                ...
                self.subtrees[local_key] = attribute(
                    local_key, self.input[this_level_key])
    ...
    def generate_code(self,
        ...
        for enum in self.subtrees.keys():
            self.output.extend(
                self.gen_single_enum(
                    enum,
                    self.subtrees[enum],
                    type=converter.lookup_enum_size("standard")))
            self.output.append("\n")
    ...
    def gen_single_enum(self, key, enumerations, type="<ubyte>"):
        ...
        try:
            ...
            addin_doc = ""
        lines.append("  " + addin + " = " + str(hex(keys[-1])) + "" +
                     addin_doc)
        lines.append("};")
        return lines
...
class data_point():
    ...
    def gen_switch(self, size=None):
        ...
        if type(switch_term.split(".")) is list:
            poss_enum_dict = self.root.lookup_enum(
                switch_term.split(".")[::-1][0], error_suppresion=True)
        ...
        for case_key in cases.keys():
            ...
            if case == ["_"] or case == "_":
                ...
                case_val = str(
                    hex(self.root.lookup_enum_val_2_key(case[0], case[1])))
            ...
            self.front.append("             " + str(cases[case_key]) +
                              "_TYPE " + str(self.id) + paramfield + ";")
        ...
        if default_needed and "size" in self.input.keys():
            ...
            self.front.append("             ubyte raw_data_CONVERTER[" +
                              str(self.input["size"]) + "];")
            self.front.append("             break;")
        elif default_needed and not switch_over_enum:
            self.front.append("         default:")
            self.front.append(
                "             ubyte raw_data_CONVERTER[LENGHT MISSING];")
            self.front.append("             break;")
    ...
    def gen_if(self, containing_type=None):
        ...
        self.front.append(
            "    if (" +
            self.root.expr_resolve(condition, translate_condition_2_c=True) +
            ") {")
    ...
    def gen_repeat(self, size=None):
        if "repeat-until" in self.this_level_keys:
            ...
            condition = self.root.expr_resolve(condition_in,
            ...
            expr = self.root.expr_resolve(expr_in,
                                          translate_condition_2_c=True)
        ...
        if self.size_eos and not forwarding and "length_CONVERTER" == size:
            ...
            if (temp_type is not None):  # BASIC TYPES
                if temp_type != "byte":
                    ...
                    self.front.append(
                        prepend + str(self.root.resolve_datatype(self.type)) +
                        " " + str(self.id) + ";" + loc_doc)
                else:
                    self.front.append(
                        prepend + str(self.root.resolve_datatype(self.type)) +
                        " " + str(self.id) + "[" +
                        str(self.root.resolve_datatype(
                            self.type, getsize=True)) + "]" + ";" + loc_doc)

            elif " " in str(self.type):
                self.type = self.root.expr_resolve(
                    self.type, translate_condition_2_c=True)
                ...
                length_addon = ""
                if "_ENUM" not in self.type and self.root.lookup_type(
                        name=self.type.split("_TYPE")[0],
                    ...
                    try:
                        ...
                        print_debug(self.input)
                        exit(-1)
                    length_addon = "(" + self.root.expr_resolve(
                        self.input["size"], translate_condition_2_c=True) + ")"
    ...
    def gen_instances(self, condition):
        ...
        for element in condition_list:
            ...
            if instance is not None and not self.root.lookup_instance(
                    element, self.id, check_if_implemented=True):
                self.root.mark_as_implemented(
                    "instance",
                    element,
                    value=True,
                    containing_type=self.containing_type)
                temp = " ".join(self.root.expr_resolve(str(instance["value"])))
                self.front.append("    local int64 " + str(element) + " = " +
                                  str(temp) + ";" +
                                  (("   //" + str(instance["doc"])
                                    ) if "doc" in instance.keys() else ""))
    ...
    def gen_contents(self):
        ...
        if self.magic_len > 1:
            self.front.append("    ubyte " + str(self.id) + "[" +
                              str(self.magic_len) + "];")
            self.front.append("    if (" + self.id + "[0] != " +
                              self.magic[0] + " ||")
            for x in range(1, self.magic_len - 1):
                self.front.append("        " + self.id + "[" + str(x) +
                                  "] != " + self.magic[x] + " ||")
            self.front.append("        " + self.id + "[" +
                              str(self.magic_len - 1) + "] != " +
                              self.magic[self.magic_len - 1] + ") {")
        else:
            self.front.append("    ubyte " + str(self.id) + ";")
            self.front.append("    if (" + self.id + " != " + self.magic[0] +
                              ") {")
        self.front.append('         Warning("Magic Bytes of ' + self.id +
                          ' not matching!");')
...
class seq(Converter):
    ...
    def generate_code(self,
        ...
        if "length_CONVERTER" == size:
            ...
            self.output.append(
                "    local uint32 UNTIL_CONVERTER = FTell() + length_CONVERTER;//TESST"
...
class instances(seq):
    ...
    def parse_subtree(self, name=None):
        for this_level_key in self.this_level_keys:
            local_key = remap_keys(this_level_key)
            if local_key is not None:
                self.root.register_instance(local_key,
                                            self.input[local_key],
                                            containing_type=name)
                self.subtrees[local_key] = data_point(self.input[local_key],
                                                      name=local_key,
                                                      parent=self,
                                                      root=self.root)
    ...
    def generate_code(self,
        ...
        for this_level_key in self.this_level_keys:
            ...
            if instance is not None and not self.root.lookup_instance(
                ...
                temp = " ".join(self.root.expr_resolve(str(instance["value"])))
                self.front.append("    local double " + str(this_level_key) +
                                  " = " + str(temp) + ";" +
                                  (("   //" + str(instance["doc"])
                                    ) if "doc" in instance.keys() else ""))
        self.output.extend(self.front)
...
class types(Converter):
    ...
    def parse_subtree(self, name=None):
        for this_level_key in self.this_level_keys:
            ...
            if local_key is not None:
                ...
                self.subtrees[local_key] = Converter(
                    self.input[this_level_key], parent=self, root=self.root)
    ...
    def gen_complete_types(self, size=None):
        ...
        for this_level_key in self.this_level_keys:
            ...
            self.pre.append("struct " + str(this_level_key) + "_TYPE" +
                            forward_lenfield + ";")
            ...
            if lenfield != "":
                output.append(
                    "    local uint32 struct_start_CONVERTER = FTell();")
...
def main():
    ...
    if len(sys.argv) != 3:
        print(
            "USAGE = python3 Converter.py <input file path> <output file path>"
        )  # TODO
        exit(1)
