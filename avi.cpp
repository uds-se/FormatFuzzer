...
public:
	...
	MainAVIHeader* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	AVIStreamHeader* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	strfHEADER_BIH* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	WAVEFORMATEX* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	strfHEADER_WAVE* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
class frame_struct {
	std::vector<frame_struct*>& instances;
...
public:
	...
	int64 _startof = 0;
	std::size_t _sizeof = 0;
	frame_struct& operator () () { return *instances.back(); }
	frame_struct* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
	frame_struct(std::vector<frame_struct*>& instances) : instances(instances) { instances.push_back(this); }
	~frame_struct() {
		if (generated == 2)
			return;
		while (instances.size()) {
			frame_struct* instance = instances.back();
			instances.pop_back();
			if (instance->generated == 2)
				delete instance;
		}
	}
	frame_struct* generate(/*local*/ std::string& movi_blk_hdr);
};
...
class LISTHEADER {
	...
	std::string id_var;
	uint32 list_datalen_var;
	std::string type_var;
	avihHEADER* avhi_var;
	strhHEADER* strh_var;
	strnHEADER* strn_var;
	frame_struct* frame_var;
	std::string data_var;

public:
	bool id_exists = false;
	bool list_datalen_exists = false;
	...
	uint32 list_datalen() {
		assert_cond(list_datalen_exists, "struct field list_datalen does not exist");
		return list_datalen_var;
	}
	std::string type() {
...
LISTHEADER& frame_struct::movi_list() {
...
public:
	...
	AVIINDEXENTRY* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
	...
	AVIINDEXENTRY* generate(std::string _type, DWORD _flags, DWORD _offset, DWORD _length);
};
...
class idx1HEADER {
	...
	AVIINDEXENTRY* index_entry_var;
	uint32 idx1_hdr_datalen_var;

public:
	bool id_exists = false;
	bool idx1_datalen_exists = false;
	bool index_entry_exists = false;
	...
	AVIINDEXENTRY& index_entry() {
		assert_cond(index_entry_exists, "struct field index_entry does not exist");
		return *index_entry_var;
	}
...
public:
	...
	VIDEO_FIELD_DESC* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	VideoPropHeader* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
public:
	...
	genericblock* operator [] (int index) {
		assert_cond((unsigned)index < instances.size(), "instance index out of bounds");
		return instances[index];
	}
...
std::vector<frame_struct*> frame_struct_frame_instances;
...
std::vector<AVIINDEXENTRY*> AVIINDEXENTRY_index_entry_instances;
...
public:
	...
	char_class ckid_element;
	char_array_class ckid;
	...
	globals_class() :
		...
		frame(frame_struct_frame_instances),
		...
		dwChunkOffset(1),
		dwChunkLength(1),
		index_entry(AVIINDEXENTRY_index_entry_instances),
		idx1_hdr_datalen(1),
...
frame_struct* frame_struct::generate(/*local*/ std::string& movi_blk_hdr) {
	if (generated == 1) {
		frame_struct* new_instance = new frame_struct(instances);
		new_instance->generated = 2;
		return new_instance->generate(movi_blk_hdr);
	}
	if (!generated)
		generated = 1;
	_startof = FTell();
...
LISTHEADER* LISTHEADER::generate() {
	...
	GENERATE_VAR(id, ::g->id.generate(4));
	datalen_pos = FTell();
	GENERATE_VAR(list_datalen, ::g->list_datalen.generate());
	...
	if (((Memcmp(type(), "movi", 4) == 0) || (Memcmp(type(), "rec ", 4) == 0))) {
		...
		do {
			...
			GENERATE_VAR(frame, ::g->frame.generate(movi_blk_hdr));
			::g->frame_index++;
		...
		FSeek(datalen_pos);
		evil_state = SetEvilBit(false);
		GENERATE_VAR(list_datalen, ::g->list_datalen.generate({ ((uint32)pointer + 4) }));
		SetEvilBit(evil_state);
		FSeek(after_pos);
	} else {
		GENERATE_VAR(data, ::g->data__.generate((list_datalen() - 4)));
	};
	};
	};
...
AVIINDEXENTRY* AVIINDEXENTRY::generate(std::string _type, DWORD _flags, DWORD _offset, DWORD _length) {
	if (generated == 1) {
		AVIINDEXENTRY* new_instance = new AVIINDEXENTRY(instances);
		new_instance->generated = 2;
		return new_instance->generate(_type, _flags, _offset, _length);
	}
	if (!generated)
		generated = 1;
	_startof = FTell();

	GENERATE_VAR(ckid, ::g->ckid.generate(4, { _type }));
	GENERATE_VAR(dwFlags, ::g->dwFlags.generate({ _flags }));
	GENERATE_VAR(dwChunkOffset, ::g->dwChunkOffset.generate({ _offset }));
	GENERATE_VAR(dwChunkLength, ::g->dwChunkLength.generate({ _length }));
...
idx1HEADER* idx1HEADER::generate() {
	...
	index_start = FTell();
	for (i = 0; (i < ::g->frame_index); i++) {
			if (::g->frame()[i]->movi_chunk_exists) {
			GENERATE_VAR(index_entry, ::g->index_entry.generate(::g->frame()[i]->movi_chunk().id(), 0x00000000, 0x00000000, ::g->frame()[i]->movi_chunk().movi_datalen()));
		} else {
			GENERATE_VAR(index_entry, ::g->index_entry.generate(::g->frame()[i]->movi_list().type(), 0x00000000, 0x00000000, ::g->frame()[i]->movi_list().list_datalen()));
		};
	;
	};
	index_end = FTell();
